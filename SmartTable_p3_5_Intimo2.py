...
def tshirtMeasuring(imgSrc):
	...
	cv2.line(rotated_frame, (height_array_x,body_height_first), (height_array_x,body_height_last), (255,0,0), 2)	# Draw height calculating line on image
	cv2.circle(rotated_frame,(height_array_x,body_height_first), 3, (0,0,255), -1)
	cv2.circle(rotated_frame,(height_array_x,body_height_last), 3, (0,0,255), -1)
	font = cv2.FONT_HERSHEY_SCRIPT_COMPLEX
	# valueHeight = (getmmDistance(pixel_height)/10) - 0.5
	...
	if 0 < body_sweap_y and body_sweap_y < height + 25:
		white = False
		first = 0
		last = 0
		for i in range(0,len(rotated_mask[body_sweap_y])):						# Calculate pixel body sweap by checking pixel value
			...
			# 	white = False
			### First time from black to white -> Updates first, Every other times from black to white -> do nothing
			### Every times from white to black -> Updates last
			if white == False and rotated_mask[body_sweap_y][i] != 0 and first == 0:
				first = i 														# First white pixel
				white = True
			elif white == True and rotated_mask[body_sweap_y][i] == 0:
				last = i-1 														# Last white pixel
		...
		font = cv2.FONT_HERSHEY_SCRIPT_COMPLEX
		# valueSweap = (getmmDistance(pixel_body_sweap)/10) + 0.5
	...
	if len(continuous_white_counts) > 0:
		...
		if len(body_width_first)>0 and len(body_width_last)>0:
			...
			if rotated == False:
				...
				cv2.circle(rotated_frame,(body_width_last[len(body_width_last)-1],(body_width_y-body_width_y_dif*step)), 3, (0,0,255), -1)
			font = cv2.FONT_HERSHEY_SCRIPT_COMPLEX
			# valueWidth = (getmmDistance(pixel_body_width_actual)/10) + 0.5
	...
	print("temp_count_pre_1 ", temp_count_pre_1)
	tempx1 = back_neck_x1
	tempx2 = back_neck_x2
	...
	# for i in range(int(1),int(pixel_body_width_actual*0.5)):		# Pre guessing a range for neck width with respect to body width
		...
		if temp_count_1 < temp_count_pre_1:
			...
			if temp_count_1_1 < temp_count_pre_2:
				...
				back_neck_x1 = tempx1
				back_neck_x2 = height_array_x - (tempx1 - height_array_x)
				print("temp_count pre 2 ", temp_count_pre_2)
				print("break ", back_neck_x1)
				print("break ", back_neck_x2)
				break
			else:
	...
	# for i in range(int(20),int(pixel_body_width_actual*0.5)):
...
# 	whiteMode = wM
	...
	cap = cv2.VideoCapture("E:\SmartTable_Test\WIN_20191008_15_39_58_Pro.mp4")
