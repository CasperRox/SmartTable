import sys
import SmartTable_p3_2_GUI_support
import SmartTable_p3_2
...
def vp_start_gui():
	...
	SmartTable_p3_2_GUI_support.init(root, top)
	root.resizable(0,0)
	# root.after(100, SmartTable_p3_2.loopTest)
	root.mainloop()
...
def create_Smart_Table(root, *args, **kwargs):
	...
	w = Toplevel (root)
	top = Smart_Table (w)
	SmartTable_p3_2_GUI_support.init(w, top, *args, **kwargs)
	return (w, top)
...
class Smart_Table:
	...
	def runMeasuring(self):
		...
		# 	messagebox.showerror("Input Error", "Please enter valid Measurement Values for all fields")
			...
			SmartTable_p3_2.getMeasurements(sN, sz, bH, bHT, bW, bWT, bS, bST, bNW, bNWT, whiteMode)
	...
	def __init__(self, top=None):
		...
		# self.btnRun.configure(command=SmartTable_p3_2.getMeasurements)
		...
		# self.btnStop.configure(command=SmartTable_p3_2.testing)
